...
class TeamsUIMethods:
    ...
    def click_captions_button(self):
        ...
        if closed_caption_enable_result:
            ...
            closed_caption_set_language_result = self.driver.execute_script("return window.callManager?.setClosedCaptionsLanguage('es-mx')")
            if closed_caption_set_language_result:
                logger.info("Closed captions language set programatically")
            else:
                logger.info("Failed to set closed captions language programatically")
