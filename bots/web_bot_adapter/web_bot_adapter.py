    ...
    def repeatedly_attempt_to_join_meeting(self):
        ...
        while num_retries <= max_retries:
            try:
                ...
                if num_retries >= max_retries:
                    logger.info(f"Failed to join meeting and the unexpected {e.__class__.__name__} exception with message {e.message} is retryable but the number of retries exceeded the limit, so returning.")
                    self.send_debug_screenshot_message(step="unknown", exception=e, inner_exception=None)
                    return

                if self.left_meeting or self.cleaned_up:
                    logger.info(f"Failed to join meeting and the unexpected {e.__class__.__name__} exception with message {e.message} is retryable but the bot has left the meeting or cleaned up, so returning.")
                    return

                logger.info(f"Failed to join meeting and the unexpected {e.__class__.__name__} exception with message {e.message} is retryable so retrying")
