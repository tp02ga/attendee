...
class CaptionEntry:
    ...
    def should_upsert_to_db(self, should_flush=False) -> bool:
        ...
        if not self.last_upsert_to_db_at:
            return ((datetime.utcnow() - self.created_at) > timedelta(seconds=5)) or should_flush

        # If modified since last upsert to db and hasn't been updated recently
        return self.modified_at > self.last_upsert_to_db_at and (((datetime.utcnow() - self.modified_at) > timedelta(seconds=5)) or should_flush)
