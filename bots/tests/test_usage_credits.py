...
class TestCreditTransactions(TransactionTestCase):
    ...
    def test_credit_transaction_creation(self):
        ...
        transaction = CreditTransactionManager.create_transaction(
            organization=self.organization,
            centicredits_delta=-self.bot.centicredits_consumed(),
            bot=self.bot,
...
class TestTransactionIntegrity(TransactionTestCase):
    def setUp(self):
        ...
        self.bot1 = Bot.objects.create(
            project=self.project,
            name="Bot One",
            meeting_url="https://test.com/one"
        )
        
        self.bot2 = Bot.objects.create(
            project=self.project,
            name="Bot Two",
            meeting_url="https://test.com/two"
        )
    
    def test_transaction_chaining(self):
        """Test that transactions are properly chained with parent-child relationships"""
        # Create first transaction
        transaction1 = CreditTransactionManager.create_transaction(
            organization=self.organization,
            centicredits_delta=-100,
            bot=self.bot1,
            description="Bot One usage"
        )
        
        # Create second transaction
        transaction2 = CreditTransactionManager.create_transaction(
            organization=self.organization,
            centicredits_delta=-50,
            bot=self.bot2,
            description="Bot Two usage"
        )
        ...
        transaction3 = CreditTransactionManager.create_transaction(
            organization=self.organization,
